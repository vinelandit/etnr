    ...
    switch ($command) {
        ...
        case 'matchtree':
            if(isset($payload['lat'])&&isset($payload['lon'])) {
                $lat = $payload['lat'];
                $lon = $payload['lon'];
                $sq = "SELECT * FROM ( SELECT *, ( ( ( acos( sin(( $lat * pi() / 180)) * sin(( `lat` * pi() / 180)) + cos(( $lat * pi() /180 )) * cos(( `lat` * pi() / 180)) * cos((( $lon - `lon`) * pi()/180))) ) * 180/pi() ) * 60 * 1.1515 * 1.609344 ) as distance FROM `trees` ) trees WHERE CommonName IS NOT NULL AND distance <= 10 ORDER BY distance ASC LIMIT 5";
        ...
        case 'uploadmedia':
            ...
            if(isset($payload['mimetype']) && isset($payload['data']) && isset($payload['mediaid']) && isset($payload['lat']) && isset($payload['lon'])) {
                ...
                if($result->num_rows==1) {
                    ...
                    // insert
                    $sq = "INSERT INTO userMedia (userID,mediaid,url,lat,lon,mimetype) VALUES
                            ($id,'".$payload['mediaid']."','".$url."',".$payload['lat'].",".$payload['lon'].",'".$payload['mimetype']."')";
